...
def compute_ue_flows(max_iterations=1000, epsilon=0.001, adj=None, demand=None, link_data=None):
...
def worker(args):
    ...
    for link_name, option in combination.items():
        if option == 0:
            continue
        if link_name.endswith('_1') or link_name.endswith('_2'):
            link_prefix = link_name[:-2]  # e.g., 'A#1' from 'A#1_1'
            from_node, to_node, _, free_flow_time, capacity, cost = new_links[link_prefix][option-1]
            total_cost += cost
    ...
    flows, travel_time = compute_ue_flows(adj=adj, demand=demand, link_data=link_data)
    total_tt = sum(flows[lid] * travel_time[lid] for lid in flows)
    return total_tt, combination

# Compute UE without improvements
flows_no_improvements, travel_time_no_improvements = compute_ue_flows(adj=adj, demand=demand, link_data=link_data)
total_tt_no_improvements = sum(flows_no_improvements[lid] * travel_time_no_improvements[lid] for lid in flows_no_improvements)

# Output equilibrium results without improvements
print("\nEquilibrium Results (No Improvements):")
for lid, x in flows_no_improvements.items():
    t = travel_time_no_improvements[lid]
    v_c = x / link_data[lid][1]
    print(f"Link {lid}: volume={x:.1f} veh/hr, travel_time={t:.1f} min, v/c={v_c:.2f}")

# Generate all possible combinations
options = {
    'A#1_1': [0, 1],  # 0: none, 1: build
    'A#1_2': [0, 1],
    'A#2_1': [0, 1],
    'A#2_2': [0, 1],
    'B#1_1': [0, 1],
...
if __name__ == '__main__':
    ...
    for link_name, option in best_combination.items():
        ...
        if link_name.endswith('_1') or link_name.endswith('_2'):
            link_prefix = link_name[:-2]  # e.g., 'A#1' from 'A#1_1'
            _, _, _, _, _, cost = new_links[link_prefix][option-1]
            total_cost += cost
            print(f"Build {link_name}: Cost = ${cost}M")
        else:
            link_id = int(link_name)
    ...
    print(f"Total System Travel Time with Improvements: {best_travel_time:.2f} vehicle-minutes")
    print(f"Total System Travel Time without Improvements: {total_tt_no_improvements:.2f} vehicle-minutes")
    print(f"Reduction in Total System Travel Time: {(total_tt_no_improvements - best_travel_time):.2f} vehicle-minutes")