...
# --- Frank-Wolfe Algorithm with Tolls ---
def all_or_nothing_assign(travel_time, tolls=None):
    if tolls is None:
        tolls = {lid: 0 for lid in link_data}
    ...
    for origin, dests in demand.items():
        ...
        while pq:
            ...
            if d > dist[u]:
                continue
            for v, lid in adj[u]:
                cost = travel_time[lid] + tolls[lid]
                new_d = d + cost
                if new_d < dist[v]:
                    dist[v] = new_d
                    prev[v] = (u, lid)
...
# Line search helper function: computes dZ/dalpha with tolls
def compute_dZ_dalpha(x, y, alpha_step, tolls, link_data):
    sum_dZ = 0
...
# Line search using bisection method with tolls
def line_search(x, y, tolls, link_data, tol=1e-6):
    alpha_low = 0
    alpha_high = 1
    dZ0 = compute_dZ_dalpha(x, y, 0, tolls, link_data)
    if dZ0 >= 0:
        return 0
    dZ1 = compute_dZ_dalpha(x, y, 1, tolls, link_data)
    if dZ1 <= 0:
        return 1
    while alpha_high - alpha_low > tol:
        alpha_mid = (alpha_low + alpha_high) / 2
        dZ_mid = compute_dZ_dalpha(x, y, alpha_mid, tolls, link_data)
        if dZ_mid > 0:
...
# Function to compute UE flows with given tolls
def compute_ue_flows(tolls, max_iterations=1000, epsilon=0.001):
    travel_time = {lid: t0 for lid, (t0, cap) in link_data.items()}
    flows = all_or_nothing_assign(travel_time, tolls)
    prev_flows = flows.copy()
    for it in range(1, max_iterations + 1):
        # Update travel times
        for lid, (t0, cap) in link_data.items():
            travel_time[lid] = t0 * (1 + alpha * (flows[lid] / cap)**beta)
        # All-or-nothing assignment with tolls
        new_flows = all_or_nothing_assign(travel_time, tolls)
        # Line search for optimal alpha
        alpha_opt = line_search(flows, new_flows, tolls, link_data)
        # Update flows
        for lid in flows:
        ...
        if relative_gap < epsilon:
            print(f"UE converged after {it} iterations with relative gap {relative_gap:.6f}")
            break
        prev_flows = flows.copy()
    else:
        print(f"UE reached maximum iterations ({max_iterations}) with relative gap {relative_gap:.6f}")
    return flows, travel_time

# Compute UE without tolls
tolls_no_tolls = {lid: 0 for lid in link_data}
flows_no_tolls, travel_time_no_tolls = compute_ue_flows(tolls_no_tolls)

# Output equilibrium results without tolls
print("\nEquilibrium Results (No Tolls):")
for lid, x in flows_no_tolls.items():
    t = travel_time_no_tolls[lid]
    v_c = x / link_data[lid][1]
    print(f"Link {lid}: volume={x:.1f} veh/hr, travel_time={t:.1f} min, v/c={v_c:.2f}")

# Identify bottleneck links (v/c >= 0.90)
bottleneck_links = {lid: x / link_data[lid][1] for lid, x in flows_no_tolls.items() if x / link_data[lid][1] >= 0.90}
print("\nBottleneck Links (v/c >= 0.90):")
for lid, vc in bottleneck_links.items():
    print(f"Link {lid}: v/c ratio = {vc:.2f}")

# Calculate flow-to-capacity ratios and select top 5 congested links
vc_ratios = {lid: flows_no_tolls[lid] / link_data[lid][1] for lid in link_data}
selected_links = sorted(vc_ratios, key=vc_ratios.get, reverse=True)[:5]
print("\nSelected Links for Tolling (Top 5 by v/c ratio):")
for link_id in selected_links:
    print(f"Link {link_id}: v/c ratio = {vc_ratios[link_id]:.2f}")

# Define toll values and combinations
step_size = 0.5
toll_values = [i * step_size for i in range(int(3.0 / step_size) + 1)]  # [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0]
toll_combinations = list(itertools.product(toll_values, repeat=len(selected_links)))

# Grid search for optimal tolls
best_travel_time = float('inf')
best_tolls = None

for combination in toll_combinations:
    tolls = {lid: 0.0 for lid in link_data}
    for link_id, toll_value in zip(selected_links, combination):
        tolls[link_id] = toll_value
    flows_with_tolls, travel_time_with_tolls = compute_ue_flows(tolls)
    total_tt_with_tolls = sum(flows_with_tolls[lid] * travel_time_with_tolls[lid] for lid in flows_with_tolls)
    if total_tt_with_tolls < best_travel_time:
        best_travel_time = total_tt_with_tolls
        best_tolls = tolls.copy()

# Compute total system travel time without tolls
total_tt_no_tolls = sum(flows_no_tolls[lid] * travel_time_no_tolls[lid] for lid in flows_no_tolls)

# Output results
print("\nOptimal Toll Values:")
for link_id in selected_links:
    print(f"Link {link_id}: {best_tolls[link_id]:.1f} minutes")
print(f"Total System Travel Time with Optimal Tolls: {best_travel_time:.2f} vehicle-minutes")
print(f"Total System Travel Time without Tolls: {total_tt_no_tolls:.2f} vehicle-minutes")
print(f"Reduction in Total System Travel Time: {(total_tt_no_tolls - best_travel_time):.2f} vehicle-minutes")