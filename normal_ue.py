...
# --- Network Data Setup ---
# Link characteristics: free-flow time (min) and capacity (veh/hr) for each directed link
link_data = {
...
# Adjacency list representation of the directed network (from-node -> list of (to-node, link_id))
network_edges = [
...
# OD demand matrix (veh/hr) for origin->destination
demand = {
...
def all_or_nothing_assign(travel_time):
    ...
    for origin, dests in demand.items():
        ...
        # Assign flow to shortest path for each destination
        for dest, vol in dests.items():
            if vol == 0: continue
            # backtrack from dest to origin
            node = dest
...
# Initial travel times = free-flow times
travel_time = {lid: t0 for lid,(t0,cap) in link_data.items()}
flows = all_or_nothing_assign(travel_time)

# Iterative update
for it in range(1, 1001):
    # Update travel times based on current flows
    for lid, (t0, cap) in link_data.items():
        # BPR function
        travel_time[lid] = t0 * (1 + alpha * (flows[lid]/cap)**beta)
    # All-or-nothing assignment on updated times
    new_flows = all_or_nothing_assign(travel_time)
    # Line search (using Method of Successive Averages for simplicity)
    theta = 1.0 / (it + 1)
    # Update flows
    for lid in flows:
        flows[lid] = flows[lid] + theta * (new_flows[lid] - flows[lid])
    # (Convergence check: one could compute gap or check changes in flows/travel_time here)
...
bottleneck_links = {lid: x / link_data[lid][1] for lid, x in flows.items() if x / link_data[lid][1] > 0.90}
